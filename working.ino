...
void controlMotor(uint8_t motorIndex, int rcValue) {
  ...
  if (motorIndex == 0) { // CH1 - STEERING CHANNEL
    activeDeadband = 5; // ULTRA-LOW deadband for precise steering
  } else {
  ...
  if (motorIndex == 0) { // CH1 - STEERING OPTIMIZATION
    // ULTRA-AGGRESSIVE steering: Use only 1/4 of stick travel for full power
    effectiveRange = (forward ? (rcMax[motorIndex] - rcCenter[motorIndex]) : (rcCenter[motorIndex] - rcMin[motorIndex])) / 4;
    minSpeed = 80; // HIGH starting speed for instant steering response
    maxSpeed = 255; // Full power for steering
    
    // Ensure minimum effective range for steering
    if (effectiveRange < 80) effectiveRange = 80;
  ...
  if (motorIndex == 0) { // CH1 - STEERING
    // Apply CUBIC curve for ultra-aggressive steering response
    float normalizedSpeed = (float)motorSpeed / 255.0;
    normalizedSpeed = normalizedSpeed * normalizedSpeed * normalizedSpeed; // Cubic curve
